...
class SecureStorage {
        ...
        return new Promise((resolve, reject) => {
            try {
                gapi.load('client:auth2', () => {
                    gapi.client.init({
                        apiKey: 'AIzaSyCBEiN7MzQLJ5mNjubdhjzBMys1Z_MwmZU', // Substituir por sua chave de API
                        clientId: '407660085834-8s0pohe42hiu474vhd617a84bvnmq46m.apps.googleusercontent.com', // Substituir por seu Client ID
                ...
                console.error('Erro ao carregar Google Drive API:', e);
                reject(e);
            }
        });
    }
    ...
    async syncWithDrive() {
        ...
        if (!this.driveInitialized || !this.driveClient || !this.driveClient.isSignedIn.get()) {
        ...
        try {
            ...
            // Criptografar dados para upload
            const dataStr = JSON.stringify(this.data);
            const encryptedData = this.encrypt(dataStr, this.currentPin);
    ...
    async restoreFromDrive() {
        ...
        try {
            ...
            if (!this.driveFileId) {
                throw new Error('Nenhum backup encontrado no Drive');
            }
